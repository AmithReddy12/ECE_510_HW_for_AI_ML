...
Webcam frame is captured using OpenCV.
Frame is converted to grayscale and resized to 48x48.
DeepFace.analyze() is used to predict emotion.
Results are logged with inference time into inference_log.csv.
Hardware Mode (Verilator Simulation)
Frame is preprocessed (grayscale, resized, normalized).
Pixel data is saved to input_image.txt.
Python script invokes the Verilator simulation using subprocess.run().
sim_main.cpp reads pixel data and simulates cnn_accelerator.
When valid_out goes high, it prints the predicted emotion class.
Python reads the prediction, maps class index to emotion label.
Logs result and timing to inference_log.csv and emotion_predictions.csv.
Output Logs
inference_log.csv

Columns: Frame, Emotion_HW, Time_HW, Emotion_SW, Time_SW
